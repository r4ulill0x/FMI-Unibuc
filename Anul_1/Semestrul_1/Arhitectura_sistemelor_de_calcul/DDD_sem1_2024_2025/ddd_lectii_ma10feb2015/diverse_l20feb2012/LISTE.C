...
int cauta(struct nod *l, int x, int n){
  ...
  if(n==0) return poz; else return -1;
}
...
struct nod * eliminare(struct nod *l, int n){
  ...
  if(n<0)return NULL;
  if(!l)return NULL;
  if(n==0){aux1=l; l=l->leg; free(aux1); return l;}
  for(aux2=l; aux2->leg && n>1; aux2=aux2->leg, --n);
  if(n>1)return NULL;
  aux1=aux2->leg;
  if(aux1)aux2->leg=aux1->leg;
  free(aux1);
  return l;
}
...
struct nod * inserare1(struct nod *l, int x, int y, int op){
 ...
 if(op==1){          /* inserare inainte de y */
  ...
  for(aux2=l; aux2->leg; aux2=aux2->leg)
    if(aux2->leg->inf==y)break;
  if(!aux2->leg)return NULL;
  aux1=nodnou();
  aux1->inf=x; aux1->leg=aux2->leg;
  aux2->leg=aux1;
  return l;
 }else if(op==2){    /* inserare dupa y */
  ...
  aux2->leg=aux1;
  return l;
 }else return NULL;  /* optiune gresita */
}

/* varianta scrisa deasupra primitivelor cu n */
struct nod * inserare2(struct nod *l, int x, int y, int op){
 int n; struct nod * aux;
 n=cauta(l,y,1);
/*  if(n==-1)return NULL; */
 switch(op){
   case 1: aux=inserare(l,x,n); break;   /* inserare inainte de y */
   case 2: aux=inserare(l,x,n+1); break; /* inserare dupa y */
   default: aux=NULL;
 }
 return aux;
}
...
int pop(stiva *s){
  struct nod *aux; int x;
  x=(*s)->inf;
  aux=*s; *s=(*s)->leg; free(aux);
  return x;
}

/* golire stiva */
void golire(stiva *s){ while(!vida(s))pop(s); }

/* citire stiva */
void cit(stiva *s){
  int x;
  golire(s);
  for(scanf("%d",&x); x!=-1; scanf("%d",&x)) push(s,x);
}
...
void scr(stiva *s){
 stiva s1; int x;
 init(&s1);
 while(!vida(s)){x=pop(s); push(s1,x);}
 while(!vida(s1)){x=pop(s1); push(s,x); printf("%d ",x);}
 printf("\n");
}
...
void cop(stiva *d, stiva *s){
 ...
 golire(d);
 while(!vida(s)){x=pop(s); push(s1,x);}
 while(!vida(s1)){x=pop(s1); push(s,x); push(d,x);}
}
...
/* variabile auxiliare */
 ...
 printf("Nr. elementelor: "); scanf("%d",&n);
 if(n==0)l1=NULL;
 else{
   if((l1=(struct nod *)malloc(sizeof(struct nod)))==NULL)
     {printf("Eroare la alocare\n"); exit(1);}
   printf("Elementul nr. 1: "); scanf("%d",&l1->inf); l1->leg=NULL;
   aux1=l1;
   for(i=1;i<n;++i){
     if((aux1->leg=(struct nod *)malloc(sizeof(struct nod)))==NULL)
       {printf("Eroare la alocare\n"); exit(1);}
     printf("Elementul nr. %d: ",i+1); scanf("%d",&aux1->leg->inf);
     aux1->leg->leg=NULL;
     aux1=aux1->leg;
   }
 }
 ...
 for(i=0;i<n;++i){
   if((aux1=(struct nod *)malloc(sizeof(struct nod)))==NULL)
     {printf("Eroare la alocare\n"); exit(1);}
   printf("Elementul nr. %d: ",i+1); scanf("%d",&aux1->inf); aux1->leg=l2;
   l2=aux1;
 }
 ...
 for(scanf("%d",&x); x!=-1; scanf("%d",&x)){
   if((aux1=(struct nod *)malloc(sizeof(struct nod)))==NULL)
     {printf("Eroare la alocare\n"); exit(1);}
   aux1->inf=x; aux1->leg=l3;
   l3=aux1;
 }

 /* afisare */
 for(aux1=l1; aux1; aux1=aux1->leg) printf("%d ",aux1->inf);
 printf("\n");
 for(aux1=l2; aux1; aux1=aux1->leg) printf("%d ",aux1->inf);
 printf("\n");
 for(aux1=l3; aux1; aux1=aux1->leg) printf("%d ",aux1->inf);
 printf("\n");

 /* stergere */
 while(l3){aux1=l3; l3=l3->leg; free(aux1);}

 /*** prelucrari cu functii ***/

 /* cautarea tuturor aparitiilor */
 printf("Nr. cautat: "); scanf("%d",&x);
 ...
 printf("Elementul inainte/dupa care se insereaza: "); scanf("%d",&y);
 printf("Elementul inserat: "); scanf("%d",&x);
...
/* operatii cu stive */
 ...
 printf("Stiva citita este: "); scr(&s1); printf("\n");
 printf("Stiva citita este: "); scr(&s1); printf("\n");

 cop(&s2,&s1);

 printf("Stiva copie este: "); scr(&s2); printf("\n");
 printf("Stiva initiala este: "); scr(&s1); printf("\n");

 golire(&s1);

 printf("Stiva initiala dupa golire este: "); scr(&s1); printf("\n");
